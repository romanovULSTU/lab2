"""
18. Вычислить сумму знакопеременного ряда (-1)n-1*(|х(2n+1)|)/(2n+1)!, где х-матрица ранга к (к и матрица задаются случайным образом),
...
while abs(fraction) > accuracy:
    delta += summa
    if n % 2 == 1:
        summa += -1 * ((np.linalg.det(linalg.matrix_power(matrix, 2 * n + 1))) / fact)
    else:
        summa += 1 * ((np.linalg.det(linalg.matrix_power(matrix, 2 * n + 1))) / fact)
    n += 1
    fact = fact * (2 * n + 1) * (2 * n )
